...
- Keep each PDK's core rail pair electrically separate from its IO rail pair: IHP `VDD/VSS` and `IOVDD/IOVSS`, Sky130 `VCCD/VSSD` and `VDDIO/VSSIO`, GF180 `VDD/VSS` and `DVDD/DVSS`, ICS55 `VDD/VSS` and `VDD25/VSSD`. Do not merge rails in RTL, PDN Tcl, package diagrams or macro integration.
...
- ICS55 has `no-pll` and `pll` Tier0 variants for every QFN profile. Both use H7CR standard cells, commercial SP55 `PB4` functional pads, `PADI30/PADO30` physical package-pad overlays, supply pads, corner cells and fillers; `PB24` is not used. The `pll` variant additionally uses `PXWE1`, `PLL_TOP`, `PVDD1CAP/PVSS1CAP` and `PVDD3AP/PVSS3AP`. `PVDD1/VDD` and `PVSS1/VSS` serve the core pair; PB4 keeps `VDD25/VSSD` as a separate IO pair. `PLL_TOP` keeps `AVDD/AVSS` separate from core rails and exposes its raw control interface to Tier1. Its LibreLane PDK is an external manual adapter at `ICS55_PDK_ROOT` (default `~/.ciel/manual`) and must not add PDK installation logic to this repository.
- The ICS55 IO LEF does not provide complete package-level `VDD25/VSSD` or RDL supply abstracts. Do not call an ICS55 result signed off while PSM reports connectivity errors; resolve this with foundry package/RDL and supply-pin abstracts, not by suppressing checks or creating fictional geometry.
