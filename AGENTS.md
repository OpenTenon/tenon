...
- ICS55 has `no-pll` and `pll` Tier0 variants for every QFN profile. QFN32 no-PLL uses H7CR and commercial SP55 `PB4` functional pads with `PADI30/PADO30` overlays, `PVDD1/PVSS1/PVDD2/PVSS2`, PCORNER and PFILL cells; `PB24` is not used. Its physical `VDD25/VSSD` pins map to logical `IOVDD/IOVSS`; `FP/FPB` stay separate internal special nets. The core mesh is only `VDD/VSS`; the IO domain closes through abutted supply pads and fillers in the pad row. QFN64/QFN88/QFN128 no-PLL retain P65/ICSIOA `P65_1233_PBMUX` at DS1/DS0=`1/0` (8 mA), P65 supply pads, corners and fillers. PLL variants use PB4 plus `PXWE1`, `PLL_TOP`, `PVDD1CAP/PVSS1CAP` and `PVDD3AP/PVSS3AP`; `PLL_TOP` keeps `AVDD/AVSS` separate while exporting raw controls to Tier1. Its LibreLane PDK is an external manual adapter at `ICS55_PDK_ROOT` (default `~/.ciel/manual`) and must not add PDK installation logic to this repository.
- SP55 LEF lacks complete package-level `VDD25/VSSD` and RDL supply geometry. `harden-ics55-qfn32-no-pll-unsigned-io-pg` requires `UNSIGNED_IO_PG=1`, runs `Checker.PowerGridViolations` and retains its reports but makes PSM violations non-fatal. It skips only `OpenROAD.IRDropReport`, which otherwise reruns the same incomplete PSM check without a non-fatal control. It is never a signoff result. Resolve this only with foundry package/RDL and supply-pin abstracts; do not suppress connectivity or route-DRC checks or describe the result as clean.
