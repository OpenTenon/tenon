...
module spi_master (
  ...
  always_ff @(posedge clk) begin
    if (!rst_n) begin shift <= 8'd0; divider <= 3'd0; bit_count <= 4'd0; busy <= 1'b0; sclk <= 1'b0; end
    else if (ena) begin
      if (!busy && uio_in[0]) begin shift <= ui_in; divider <= 3'd0; bit_count <= 4'd0; busy <= 1'b1; sclk <= 1'b0; end
      else if (busy) begin
