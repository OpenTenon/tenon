...
def report_row(design: dict[str, str]) -> tuple[str, bool]:
    ...
    psm = metrics.get("design__power_grid_violation__count")
    disconnected = metrics.get("design__critical_disconnected_pin__count")
    flow_errors = metrics.get("flow__errors__count")
    passed = (
        isinstance(die_area, (int, float))
        and die_area < DIE_AREA_LIMIT_UM2
        and flow_errors == 0
        and route_drc == 0
        and antenna == 0
        and psm == 0
    ...
    return (
        ...
        + " | ".join(
            (
                ...
                metric_text(
                    die_area / 1_000_000 if isinstance(die_area, (int, float)) else None,
                    6,
                ),
                metric_text(metrics.get("design__core__area")),
                metric_text(metrics.get("design__instance__count__stdcell")),
                metric_text(route_drc),
                metric_text(antenna),
                metric_text(psm),
                metric_text(disconnected),
                metric_text(metrics.get("timing__setup__ws")),
