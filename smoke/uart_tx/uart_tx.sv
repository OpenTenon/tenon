...
module uart_tx (
  ...
  always_ff @(posedge clk) begin
    if (!rst_n) begin frame <= 10'h3ff; bit_index <= 4'd0; baud_count <= 4'd0; busy <= 1'b0; end
    else if (ena) begin
      if (!busy && uio_in[0]) begin frame <= {1'b1, ui_in, 1'b0}; bit_index <= 4'd0; baud_count <= 4'd0; busy <= 1'b1; end
      else if (busy) begin
