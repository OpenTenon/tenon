...
def ics55_geometry(profile: dict, ics55: dict, variant: dict) -> dict[str, object]:
    ...
    io_pdn = {
        "iovdd_ring_offset_um": 250.0,
        "iovss_ring_offset_um": 300.0,
        "rdl_horizontal_width_um": 35.0,
        "core_rdl_bridge_width_um": 35.0,
    }
    if not variant["include_pll"]:
        ...
        if override is not None:
            ...
            io_pdn.update(override.get("io_pdn", {}))
    core_side = die_side - 2 * core_offset
    if core_side <= 0:
        raise ValueError(f"{profile['id']} {variant['id']}: non-positive core area")
    if compact:
        core_bridge_width = float(io_pdn["core_rdl_bridge_width_um"])
        io_width = float(io_pdn["rdl_horizontal_width_um"])
        iovdd_offset = float(io_pdn["iovdd_ring_offset_um"])
        iovss_offset = float(io_pdn["iovss_ring_offset_um"])
        pad_inner_edge = float(floorplan["pad_height_um"])
        if iovdd_offset - io_width / 2 <= pad_inner_edge:
            raise ValueError(f"{profile['id']} {variant['id']}: IOVDD ring overlaps pads")
        if core_bridge_width <= 0:
            raise ValueError(f"{profile['id']} {variant['id']}: invalid core bridge width")
        if iovss_offset <= iovdd_offset:
            raise ValueError(f"{profile['id']} {variant['id']}: invalid IO ring ordering")
        # VSS uses the outer core-ring track: 4 um offset, 4 um half-width,
        # and a 10 um VDD/VSS separation from the core boundary.
        if core_offset <= iovss_offset + io_width / 2 + 18.0:
            raise ValueError(f"{profile['id']} {variant['id']}: IO and core rings overlap")
    return {
        "compact": compact,
        "die_side": die_side,
...
def render_ics55_qfn32_no_pll_fillers(fillers: dict[str, dict[str, list[str]]]) -> str:
    lines = [
        "// Generated by tools/generate_tier0.py. Do not edit manually.",
...
def render_ics55_io_pdn_overlay(geometry: dict[str, object]) -> str:
    """Render QFN-specific IO package-ring geometry for the shared PDN Tcl."""
    io_pdn = geometry["io_pdn"]
    return (
        "# Generated by tools/generate_tier0.py. Do not edit manually.\n"
        "set ::tenon_ics55_iovdd_ring_offset_um "
        f"{sky130_tcl_number(float(io_pdn['iovdd_ring_offset_um']))}\n"
        "set ::tenon_ics55_core_rdl_bridge_width_um "
        f"{sky130_tcl_number(float(io_pdn['core_rdl_bridge_width_um']))}\n"
        "set ::tenon_ics55_iovss_ring_offset_um "
        f"{sky130_tcl_number(float(io_pdn['iovss_ring_offset_um']))}\n"
        "set ::tenon_ics55_io_rdl_horizontal_width_um "
        f"{sky130_tcl_number(float(io_pdn['rdl_horizontal_width_um']))}\n"
    )

def ics55_path(relative: str) -> str:
    return f"u_reference.u_padframe.{relative}".replace("[", r"\[").replace("]", r"\]")
    ...
    for side_name, spec_side in zip(("PAD_SOUTH", "PAD_EAST", "PAD_NORTH", "PAD_WEST"), SIDES, strict=True):
        ...
        if compact:
            ...
            for index, cell in enumerate(compact_fillers[spec_side]["leading"]):
                ...
                place(f"{wrapper_prefix}u_compact_qfn32.u_fillers.u_{spec_side}_leading_{index}", x, y, orientation)
        ...
        if compact:
            for fill_index, cell in enumerate(compact_fillers[spec_side]["trailing"]):
                x, y = ics55_origin(side_name, scalar, die_side, pad_height)
                place(f"{wrapper_prefix}u_compact_qfn32.u_fillers.u_{spec_side}_trailing_{fill_index}", x, y, orientation)
                scalar += filler_widths[cell]
...
def render_ics55_config(
    ...
    if geometry["compact"]:
        compact_rtl = "- dir::../../rtl/tenon_tier0_padframe_ics55_qfn32_no_pll_fillers.sv\n"
        compact_filler_ignores = "\n- PFILL50\n- PFILL20\n- PFILL5\n- PFILL2\n- PFILL1\n- PFILL01\n- PFILL001"
        compact_filler_abstracts = "\n- ^PFILL50$\n- ^PFILL20$\n- ^PFILL5$\n- ^PFILL2$\n- ^PFILL1$\n- ^PFILL01$\n- ^PFILL001$"
    pll_assets = ""
    pll_rails = ""
...
P65_POWER_CELLS = {
    "IOVDD": "P65_1233_VDDIO3",
    "IOVSS": "P65_1233_VSSIO3",
    "VDD": "P65_1233_VDD3",
    "VSS": "P65_1233_VSS3",
}
P65_FILLERS = (
    ("P65_1233_FILLER50", 50.0),
    ("P65_1233_FILLER20", 20.0),
    ("P65_1233_FILLER10", 10.0),
    ("P65_1233_FILLER5", 5.0),
    ("P65_1233_FILLER2", 2.0),
    ("P65_1233_FILLER1", 1.0),
    ("P65_1233_FILLER01", 0.1),
    ("P65_1233_FILLER001", 0.01),
)


def build_ics55_p65_records(spec: dict, profile: dict) -> list[dict]:
    """Apply the native P65 no-PLL package contract to a common profile."""
    records = [dict(record) for record in build_records(spec, profile)]
    for record in records:
        if record["function"] in P65_POWER_CELLS:
            stem = record["instance"].replace(".u_pad", ".u_cell")
            record.update({
                "cell": P65_POWER_CELLS[record["function"]],
                "physical_instance": f"{stem}.u_pad",
                "base_width": 65.0,
            })
        elif record["function"] == "GPIO":
            gpio_index = record["rtl_signal"].removeprefix("gpio[").removesuffix("]")
            record.update({
                "cell": "P65_1233_PBMUX",
                "physical_instance": f"u_gpio_pads[{gpio_index}].u_cell.u_pad",
                "base_width": 65.0,
            })
        else:
            record.update({
                "cell": "P65_1233_PBMUX",
                "physical_instance": f"{record['instance']}.u_pad",
                "base_width": 65.0,
            })
    return records


def p65_geometry(profile: dict, ics55: dict) -> dict[str, float]:
    floorplan = ics55["p65_no_pll_floorplan"]
    die_side = float(profile["die_side_um"])
    core_offset = float(floorplan["core_offsets_um"][profile["id"]])
    override = floorplan.get("overrides", {}).get(profile["id"], {})
    die_side = float(override.get("die_side_um", die_side))
    core_offset = float(override.get("core_offset_um", core_offset))
    core_side = die_side - 2 * core_offset
    if core_side <= 0:
        raise ValueError(f"{profile['id']} p65 no-pll: non-positive core area")
    return {
        "die_side": die_side,
        "core_offset": core_offset,
        "core_side": core_side,
        "pad_width": float(floorplan["pad_width_um"]),
        "pad_height": float(floorplan["pad_height_um"]),
        "corner_size": float(floorplan["corner_size_um"]),
    }


def p65_decompose_filler(width: float) -> list[str]:
    """Fill an exact P65 pad-ring edge remainder with foundry filler cells."""
    remaining = round(width, 3)
    cells: list[str] = []
    for cell, cell_width in P65_FILLERS:
        count = int((remaining + 1e-6) // cell_width)
        cells.extend([cell] * count)
        remaining = round(remaining - count * cell_width, 3)
    if abs(remaining) > 1e-6:
        raise ValueError(f"P65 filler gap {width} um cannot be decomposed")
    return cells


def p65_fillers(
    profile: dict, records: list[dict], geometry: dict[str, float]
) -> dict[str, dict[str, list[str]]]:
    usable = geometry["die_side"] - 2 * geometry["corner_size"]
    result: dict[str, dict[str, list[str]]] = {}
    for side in SIDES:
        pad_span = sum(record["base_width"] for record in sky130_side_records(records, side))
        residual = round(usable - pad_span, 3)
        if residual < 0:
            raise ValueError(f"{profile['id']} {side}: P65 pad row exceeds die edge")
        leading = round(residual / 2, 3)
        result[side] = {
            "leading": p65_decompose_filler(leading),
            "trailing": p65_decompose_filler(round(residual - leading, 3)),
        }
    return result


def render_ics55_p65_fillers(
    ...
    for profile_id, fillers in filler_layouts.items():
        lines.extend((
            f"module tenon_ics55_p65_{profile_id}_no_pll_fillers (",
            "    inout wire vdd,",
            "    inout wire vss,",
            "    inout wire vddio,",
            "    inout wire vssio",
        ...
        for side in SIDES:
            for edge in ("leading", "trailing"):
                for index, cell in enumerate(fillers[side][edge]):
                    lines.extend((
                        f"  (* keep = \"true\" *) {cell} u_{side}_{edge}_{index} (",
                        "      .VDD  (vdd),",
                        "      .VSS  (vss),",
                        "      .VDDIO(vddio),",
                        "      .VSSIO(vssio)",
...
def render_ics55_p65_placement(
    profile: dict,
    records: list[dict],
    geometry: dict[str, float],
    fillers: dict[str, dict[str, list[str]]],
) -> str:
    """Render an abutted P65 pad ring with foundry corners and fillers."""
    side_orient = {
        "PAD_SOUTH": "R0",
        "PAD_EAST": "R90",
        "PAD_NORTH": "R180",
    ...
    filler_widths = dict(P65_FILLERS)
    die_side = geometry["die_side"]
    lines = [
        "# Generated by tools/generate_tier0.py. Do not edit manually.",
        f"# Commercial ICS55 P65 no-PLL package-pad placement for {profile['id']}.",
    ]
    ...
    def place(instance: str, x: float, y: float, orientation: str) -> None:
        lines.append(
            ...
            + " "
            + sky130_tcl_number(y)
            + f"] -orientation {orientation} -status FIRM"
        )

    for side_name, side in zip(("PAD_SOUTH", "PAD_EAST", "PAD_NORTH", "PAD_WEST"), SIDES, strict=True):
        orientation = side_orient[side_name]
        scalar = geometry["corner_size"]
        prefix = f"u_p65_{profile['id']}.u_fillers.u_{side}"
        for index, cell in enumerate(fillers[side]["leading"]):
            x, y = ics55_origin(side_name, scalar, die_side, geometry["pad_height"])
            place(f"{prefix}_leading_{index}", x, y, orientation)
            scalar += filler_widths[cell]
        for record in sky130_side_records(records, side):
            x, y = ics55_origin(side_name, scalar, die_side, geometry["pad_height"])
            place(record["physical_instance"], x, y, orientation)
            scalar += record["base_width"]
        for index, cell in enumerate(fillers[side]["trailing"]):
            x, y = ics55_origin(side_name, scalar, die_side, geometry["pad_height"])
            place(f"{prefix}_trailing_{index}", x, y, orientation)
            scalar += filler_widths[cell]
        if abs(scalar - (die_side - geometry["corner_size"])) > 1e-6:
            raise ValueError(f"{profile['id']} {side}: P65 edge does not close")
    corner = geometry["corner_size"]
    for instance, x, y, orientation in (
        ("u_corner_sw", 0.0, 0.0, "R0"),
        ("u_corner_se", die_side - corner, 0.0, "R90"),
        ("u_corner_ne", die_side - corner, die_side - corner, "R180"),
        ("u_corner_nw", 0.0, die_side - corner, "R270"),
    ):
        place(instance, x, y, orientation)
    lines.append("")
    return "\n".join(lines)


def render_ics55_p65_config(
    profile: dict, records: list[dict], ics55: dict, geometry: dict[str, float]
) -> str:
    placements = "\n\n".join(
        yaml_list(f"PAD_{side.upper()}", ics55_side_pad_placement(records, side))
        for side in SIDES
    )
    die_side = sky130_tcl_number(geometry["die_side"])
    core_offset = sky130_tcl_number(geometry["core_offset"])
    core_end = sky130_tcl_number(geometry["core_offset"] + geometry["core_side"])
    return f"""# Generated by tools/generate_tier0.py. Do not edit manually.
# ICS55 H7CR commercial P65 no-PLL Tier0 reference configuration.
meta:
  version: 3
  flow: Chip
  substituting_steps:
...
DESIGN_NAME: {ics55['package_top_prefix']}{profile['id']}_no_pll
VERILOG_FILES:
- dir::../../rtl/tenon_tier0_padframe_ics55_p65.sv
- dir::../../rtl/tenon_tier0_padframe_ics55_p65_fillers.sv
- dir::../../rtl/tenon_tier0_pdk_reference.sv
- dir::../../rtl/tenon_tier0_pdk_variants.sv
EXTRA_VERILOG_MODELS:
- dir::ics55_io_blackbox.v
EXTRA_LEFS:
- pdk_dir::libs.ref/ics55_io_p65/lef/ICSIOA_N55_3P3_1P6M1TM_openroad.lef
EXTRA_GDS:
- pdk_dir::libs.ref/ics55_io_p65/gds/ICSIOA_N55_3P3_1P6M1TM.gds
LIB:
  "*_tt_1p2_25C":
  - pdk_dir::libs.ref/ics55_LLSC_H7CR/lib/ics55_LLSC_H7CR_typ_tt_1p2_25_nldm.lib
  "*_ff_1p32_m40C":
...
PAD_VERILOG_MODELS: null
EXTRA_LIBS:
- pdk_dir::libs.ref/ics55_io_p65/lib/ICSIOA_N55_3P3_tt_1p2_3p3_25c.lib
PAD_CFG: dir::pad_cfg_p65.tcl
PRIMARY_GDSII_STREAMOUT_TOOL: klayout
RUN_MAGIC_STREAMOUT: false
...
MAGIC_EXT_ABSTRACT: true
MAGIC_EXT_ABSTRACT_CELLS:
- ^P65_1233_(CORNER|FILLER.*)$
- ^P65_1233_(PBMUX|VDD3|VSS3|VDDIO3|VSSIO3)$
ERROR_ON_LVS_ERROR: true
"""
...
def expected_outputs(spec: dict, pdk_spec: dict) -> dict[Path, str]:
    ...
    p65_filler_layouts: dict[str, dict[str, dict[str, list[str]]]] = {}
    for profile in spec["profiles"]:
        ...
        for base_variant in ics55["variants"]:
            ...
            if base_variant.get("io_family") == "p65":
                p65_records = build_ics55_p65_records(spec, profile)
                p65_geometry_data = p65_geometry(profile, ics55)
                p65_filler_layouts[profile_id] = p65_fillers(
                    profile, p65_records, p65_geometry_data
                )
            if variant.get("io_family") == "p65":
                ics55_records = p65_records
                geometry = p65_geometry_data
                filler_layout = p65_filler_layouts[profile_id]
                outputs[ICS55_FLOW_DIR / f"{profile_id}-{variant_id}.yaml"] = (
                    render_ics55_p65_config(profile, ics55_records, ics55, geometry)
                )
                outputs[
                    ICS55_FLOW_DIR / "generated" / f"{profile_id}-{variant_id}_placement.tcl"
                ] = render_ics55_p65_placement(profile, ics55_records, geometry, filler_layout)
            else:
                ...
                if variant.get("pdn_mode") == "commercial-pad-row" and geometry["compact"]:
                    outputs[ROOT / "rtl" / "tenon_tier0_padframe_ics55_qfn32_no_pll_fillers.sv"] = (
                        render_ics55_qfn32_no_pll_fillers(filler_layout)
                    )
                outputs[ICS55_FLOW_DIR / f"{profile_id}-{variant_id}.yaml"] = (
    ...
    outputs[ROOT / "rtl" / "tenon_tier0_padframe_ics55_p65_fillers.sv"] = (
        render_ics55_p65_fillers(p65_filler_layouts)
    )
    return outputs
