...
def gf180_side_placement(records: list[dict], side: str) -> list[str]:
    """Return GF180 physical PadCell paths below the Tenon helper wrappers."""
    selected = [record for record in records if record["side"] == side]
    if side in {"north", "west"}:
        selected.reverse()
...
def render_gf180_config(profile: dict, records: list[dict], gf180: dict) -> str:
    ...
    placements = "\n\n".join(
        yaml_list(f"PAD_{side.upper()}", gf180_side_placement(records, side)) for side in SIDES
    )
    return f"""# Generated by tools/generate_tier0.py. Do not edit manually.
