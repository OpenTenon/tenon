...
def expected_outputs(spec: dict, pdk_spec: dict) -> dict[Path, str]:
    ...
    for profile in spec["profiles"]:
        ...
        outputs[IHP_FLOW_DIR / f"{profile_id}.yaml"] = render_ihp_config(
            profile, records
        )
        ...
        for variant in ics55["variants"]:
            ...
            filler_layout = ics55_compact_fillers(profile, ics55_records, ics55, variant, geometry)
            ...
            ] = render_ics55_placement(profile, ics55_records, ics55, variant, geometry, filler_layout)
            if profile_id == "qfn32" and variant_id == "no-pll":
                outputs[ROOT / "rtl" / "tenon_tier0_padframe_ics55_qfn32_no_pll_fillers.sv"] = (
                    render_ics55_qfn32_no_pll_fillers(filler_layout)
                )
                outputs[ICS55_FLOW_DIR / "generated" / f"{profile_id}-{variant_id}_io_pdn.tcl"] = (
                    render_ics55_io_pdn_overlay(geometry)
                )
            outputs[
