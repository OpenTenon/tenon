...
def render_ics55_placement(profile: dict, records: list[dict], ics55: dict, variant: dict) -> str:
    ...
    for side_name, spec_side in zip(("PAD_SOUTH", "PAD_EAST", "PAD_NORTH", "PAD_WEST"), SIDES, strict=True):
        ...
        while index < len(selected):
            ...
            if record.get("oscillator"):
                ...
                spans.append(float(record["base_width"]) + filler_width)
                index += 1
        ...
        while index < len(selected):
            ...
            fx, fy = ics55_origin(side_name, scalar + float(record["base_width"]), die_side, pad_height)
            place(record["filler_instance"], fx, fy, orientation)
            scalar += float(record["base_width"]) + filler_width
            index += 1
    ...
    if variant["include_pll"]:
        core_end = float(floorplan["core_offset_um"] + profile["core_side_um"])
...
def render_ics55_config(profile: dict, records: list[dict], ics55: dict, variant: dict) -> str:
    die_side = profile["die_side_um"]
    core_offset = ics55["floorplan"]["core_offset_um"]
    core_end = core_offset + profile["core_side_um"]
    placements = "\n\n".join(yaml_list(f"PAD_{side.upper()}", ics55_side_pad_placement(records, side)) for side in SIDES)
    pll_assets = ""
    pll_rails = ""
    pll_terminals = "\n"
    ...
    for profile in spec["profiles"]:
        ...
        for variant in ics55["variants"]:
            ...
            outputs[ICS55_FLOW_DIR / f"{profile_id}-{variant_id}.yaml"] = (
                render_ics55_config(profile, ics55_records, ics55, variant)
            )
            outputs[
                ICS55_FLOW_DIR / "generated" / f"{profile_id}-{variant_id}_placement.tcl"
            ] = render_ics55_placement(profile, ics55_records, ics55, variant)
            outputs[
