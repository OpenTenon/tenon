...
| ICS55 | `tenon_tier0_padframe_ics55_{no_pll,pll}` | Commercial SP55 `PB4` with PADI30/PADO30 overlays | `VDD/VSS` | `VDD25/VSSD` | `flow/ics55/qfn*-{no-pll,pll}.yaml` |
...
For the compact ICS55 QFN32 `no-pll` reference, the generated floorplan is a 960 x 960 um die with a 300 x 300 um core. It uses the commercial 30 um PB4/PVSS1/PVDD2/PVSS2 and 35 um PVDD1 pitch, fixed PADI30/PADO30 overlay offsets, PCORNER cells, and the commercial PFILL family to close every edge. The IOVDD/IOVSS package rings are placed at 240/270 um with uniform 8 um RDL/T4M2 segments. This is a validated non-signoff reference run: Magic/KLayout DRC, Magic Spice extraction and Netgen LVS were explicitly skipped, while OpenROAD route DRC, antenna, PSM and disconnected-pin checks completed.
...
```bash
make check-pdk-ics55
make harden-ics55-qfn32-no-pll ICS55_PDK_ROOT=$HOME/.ciel/manual SKIP_DRC=1
